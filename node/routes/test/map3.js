...
router.get('/:user_id', function(req, res, next) 
{
    ...
    instance.get(`https://maps.googleapis.com/maps/api/staticmap?center=${lat},${lng}&zoom=${zoom}&size=${w}x${h}&maptype=${maptype}&key=${key}`).then(response =>
    {
        ...
        res.render('test/map3', {user_id: user_id, lat: lat, lng: lng, mapimg: "data:image/png;base64," + Buffer.from(response.data, 'binary').toString('base64'), zoom: zoom, map_w: w, map_h: h});
    });
});
